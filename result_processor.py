...
def analyze_results(profile_sequence, results, satisfaction_matrix, support_matrix):
    ...
    print("Average Squared KT Distance:")
    print(get_avg_sq_kt_distance(profile_sequence, results))
    print()
    ...
    print()
    print("Total Satisfaction of Special Voter:")
    print(get_total_satisfaction_special_voter(satisfaction_matrix, support_matrix))
...
def get_perpetual_lower_quota_compliance(satisfaction_matrix, support_matrix):
    ...
    for t in range(num_rounds):
        for v in range(num_voters):
            sat = sum(satisfaction_matrix[round_ind][v] for round_ind in range(t+1))
            quota = sum(support_matrix[round_ind][v] for round_ind in range(t+1))
...
def get_perpetual_lower_quota_compliance_ratio_special_voter(satisfaction_matrix, support_matrix):
    ...
    for t in range(num_rounds):
        ...
        sat_special_voter = sum(satisfaction_matrix[round_ind][num_voters - 1] for round_ind in range(t+1))
        quota_special_voter = sum(support_matrix[round_ind][num_voters - 1] for round_ind in range(t+1))
...
def get_total_satisfaction_special_voter(satisfaction_matrix, support_matrix):
    num_voters = len(satisfaction_matrix[0])
    num_rounds = len(satisfaction_matrix)
    
    sat_special_voter_counter = 0
    for t in range(num_rounds):
        sat_special_voter_counter += satisfaction_matrix[t][num_voters - 1]
        
    
    return sat_special_voter_counter
